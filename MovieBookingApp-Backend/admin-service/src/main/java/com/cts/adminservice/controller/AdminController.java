...
public class AdminController {
	...
	@Operation(summary = "Update show tickets", description = "Update tickets for a existing ticket")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Tickets updation successful", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "400", description = "Bad request", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "404", description = "Seleted show doesn't exist", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "401", description = "Unauthorize. Only a admin can perform this operation", content = @Content(schema = @Schema(implementation = Response.class))) })
	@PutMapping("/updateTickets")
	public ResponseEntity<Response> updateTicket(
			@Parameter(description = "JWT auth token of the application user", required = true) @RequestHeader(name = "Authorization") String token, @RequestBody UpdateRequest request) {
		adminService.updateMovieTickets(token, request);
		return ResponseEntity.ok(Response.builder().status("success").message("Tickets updated successfully").build());
	}
	
	@Operation(summary = "Add Show", description = "Add a new show")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "New show addition successful", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "400", description = "Bad request", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "404", description = "Not found", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "401", description = "Unauthorize. Only a admin can perform this operation", content = @Content(schema = @Schema(implementation = Response.class))) })
	@PutMapping("/addShows")
	public ResponseEntity<Response> addShow(
			@Parameter(description = "JWT auth token of the application user", required = true) @RequestHeader(name = "Authorization") String token,
			@Parameter(description = "Add show request with all details", required = true) @RequestBody @Valid UpdateRequest request,
			BindingResult bindingResult) {
		if (bindingResult.hasErrors()) {
			return errorResponse(bindingResult.getAllErrors());
		}
		adminService.addShows(token, request);
		return new ResponseEntity<>(Response.builder().status("success").message("Show added successsfully").build(),
				HttpStatus.CREATED);
	}
	
	@Operation(summary = "Delete shows", description = "Delete an existing show")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Show deletion successful", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "400", description = "Bad request", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "404", description = "Show not found", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "401", description = "Unauthorize. Only a admin can perform this operation", content = @Content(schema = @Schema(implementation = Response.class))) })
	@DeleteMapping("/delete-show/{showId}")
	public ResponseEntity<Response> deleteShow(
			@Parameter(description = "JWT auth token of the application user", required = true) @RequestHeader(name = "Authorization") String token,
			@Parameter(description = "Id of the movie to be deleted", required = true) @PathVariable String showId) {
		adminService.deleteShow(token, showId);
		return ResponseEntity.ok(Response.builder().status("success").message("Show deleted successfully").build());
	}
	...
	public ResponseEntity<Response> deleteMovie(
		...
		return ResponseEntity.ok(Response.builder().status("success").message("Movie deleted successfully. Deleted movie is logged in kafka").build());
	}
